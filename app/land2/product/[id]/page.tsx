...
export default async function L2ProductPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const { land2, common } = await getMessages();
  const data = getProductById(Number(id));
  if (!data) notFound();

  const idx = data.id - 1;
  type SpecRow = [string, string];
  const locale = land2.catalog.products[idx] as {
      ...
      <div className="bg-[#FEF2F2] border-b border-[#FECACA]">
        <div className="max-w-6xl mx-auto px-4 py-3">
          <nav className="text-xs text-[#6B7280] flex items-center gap-2 flex-wrap">
            <Link href="/land2" className="hover:text-[#DC2626] transition-colors">{LOGO_TEXT}</Link>
            <span>›</span>
            <Link href="/land2#catalog" className="hover:text-[#DC2626] transition-colors">{land2.catalog.title}</Link>
            <span>›</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start mb-20">
          ...
          <div className="py-2">
            ...
            <div className="mt-8 grid grid-cols-2 gap-3">
              {land2.usp.items.slice(0, 4).map((item, i) => (
        ...
        {related.length > 0 && (
          <section className="border-t border-[#FECACA] pt-14">
            <h2 className="text-xl font-light text-[#111827] mb-6">
              {(land2.catalog.categories as Record<string, string>)[data.categoryKey] ?? ''}
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
              {related.map(p => (
                <Link
                  key={p.id}
                  href={`/land2/product/${p.id}`}
