      ...
      >
            ...
            <div className="space-y-4">
              {items.map(item => (
                <div key={item.id} className="flex gap-3 py-3 border-b border-[#FECACA] last:border-0">
                  <Link href={`/land1/product/${item.id}`} onClick={closeCart} className="w-16 h-16 flex-shrink-0 relative overflow-hidden bg-[#FEF2F2] block">
                    <Image src={item.image} alt={item.name} fill className="object-cover" />
                  </Link>
                  <div className="flex-1 min-w-0">
                    <Link href={`/land1/product/${item.id}`} onClick={closeCart} className="text-sm font-medium text-[#111827] leading-tight hover:text-[#DC2626] transition-colors block">{item.name}</Link>
        ...
        {items.length > 0 && (
          <div className="px-6 py-5 border-t border-[#FECACA]">
            ...
            <button
              onClick={() => { closeCart(); router.push('/land1/checkout'); }}
