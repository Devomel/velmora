...
export default function NavBar({ t }: Props) {
  ...
  return (
    <header className={`fixed left-0 right-0 z-30 transition-all duration-500 pointer-events-none border-b ${
      ...
      <div className={`max-w-7xl mx-auto flex items-center gap-6 border transition-all duration-500 pointer-events-auto ${
        scrolled
          ? 'bg-white/80 backdrop-blur-md rounded-2xl border-[#E8DDD4]/60 shadow-sm px-4 py-3'
        ...
        <button
          onClick={openCart}
          className={`relative ml-auto flex items-center gap-2 bg-[#C4704F] hover:bg-[#A85A3A] text-white px-4 py-2 transition-all duration-500 flex-shrink-0 cursor-pointer ${scrolled ? 'rounded-xl' : 'rounded-none'}`}
          aria-label={t.cart}
        >
