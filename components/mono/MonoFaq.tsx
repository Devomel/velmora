...
type Props = { t: MonoT['faq'] };

export default function MonoFaq({ t }: Props) {
    ...
    <section className="py-20 bg-[#FDFAF7]">
      <div className="max-w-3xl mx-auto px-4">
        <div className="text-center mb-12">
          ...
          <p className="text-[#6B5B4E] text-sm">{t.subtitle}</p>
        </div>
        ...
        <div className="divide-y divide-[#E8DDD4] border-y border-[#E8DDD4]">
          {t.items.map((item, i) => (
            <div key={i}>
              ...
              >
                <span className={`text-sm font-medium transition-colors leading-snug ${open === i ? 'text-[#C4704F]' : 'text-[#1A1410] group-hover:text-[#C4704F]'}`}>
                  {item.q}
                </span>
              ...
              <div className={`overflow-hidden transition-all duration-200 ${open === i ? 'max-h-60 pb-5' : 'max-h-0'}`}>
                <p className="text-sm text-[#6B5B4E] leading-relaxed">{item.a}</p>
              </div>
            </div>
          ))}
        </div>
